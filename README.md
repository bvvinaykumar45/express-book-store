...
2. **Initialize Node.js Project**: In the terminal, run `npm init -y` to initalize a Node project and create a `package.json` file.
...
4. **Listen on Port**: Use `app.listen(PORT, () => {...})` to make the serer listen on port 8000, logging a confirmation message.
...
1. **Create a Mock Book Array**: Define a simple array, `const books = [...]`, containing sample book objects. Each book should have an `id`, `title`, and `author`.
  ...
  const books = [
    ...
    {id: 1, title: 'Book Two', author: 'Author Two' },
  ];
  ```
...
2. **Retrive Book ID**: Extract the `id` from `req.params`.
3. **Find Book**: Use `books.find()` to search for the book by `id`.
4. **Handle Not Found**: If no book found, send a `404` response with a relevant message.
...
4. **Generate New ID**: Set and ID by incrementing the last book's ID or using a random method.
...
2. **Retrive Book ID**: Extract `id` from `req.params`.
3. **Find and Remove Book**: Use `books.fiilter()` to exclude the book with the matching `id`.
4. **Check Deletion**: If no book was deleted, send a `404` response.
5. **Send Confirmation**: Return a success message indication the book was deleted, or send the updated book list.

#### Step 8. Test the Routes
1. **Start the Server**: Run `node index.js` and check for the confirmation message.
2. **Use Postman or cURL**: Demostrate how to test each route using Postman or cURL:
  - GET /books
  - GET /books/:id
  - POST /books
  - DELETE /books/:id
...
This step-by-step breakdown will guide you through setting up and testing a basic Express server with routes for managing a collection of books.
